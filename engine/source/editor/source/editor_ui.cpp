...
namespace Piccolo
{
    ...
    void EditorUI::showEditorUI()
    {
        ...
        showEditorRenderSettingsWindow(&m_global_effect_window_open);
    ...
    void EditorUI::showEditorMenu(bool* p_open)
    {
        ...
        if (ImGui::BeginMenuBar())
        {
            ...
            if (ImGui::BeginMenu("Window"))
            {
                ...
                ImGui::MenuItem("Render Grobal", nullptr, &m_global_effect_window_open);
    ...
    void EditorUI::showEditorRenderSettingsWindow(bool* p_open)
    {
        ImGuiWindowFlags window_flags = ImGuiWindowFlags_None;
        ...
        if (!ImGui::Begin("Render Settings", p_open, ImGuiWindowFlags_None))
        {
            ImGui::End();
            return;
        }

        RenderResource* render_resource = static_cast<RenderResource*>(g_editor_global_context.m_render_system->getRenderResource().get());
        RenderScene* render_scene = static_cast<RenderScene*>(g_editor_global_context.m_render_system->getRenderScene().get());
        auto& render_setting = render_resource->m_render_global_effect_setting_object;
        auto& directionLight = render_scene->m_directional_light;

        if (ImGui::CollapsingHeader("DirectionLight Settings", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ImGui::DragFloat3("Light Direction", &directionLight.m_direction[0], 0.01f, -1.f, 1.f);
            ImGui::DragFloat3("Light Color", &directionLight.m_color[0], 0.01f, 0.f, 1.f);
        }

        if (ImGui::CollapsingHeader("SSAO Settings", ImGuiTreeNodeFlags_DefaultOpen))
        {
            //ImGui::DragFloat("SSAO Effect", &render_setting.SSAOEffect, 0.01f, 0.0f, 1.0f);
            //ImGui::DragFloat("SSAO Radius", &render_setting.SSAORadius, 0.01f, 0.1f, 5.0f);
            //ImGui::DragFloat("SSAO Bias", &render_setting.SSAOBias, 0.001f, 0.0f, 0.5f);
            //ImGui::DragInt("SSAO Sample Count", &render_setting.SSAOKernelSize, 1, 16, 64);
            // 
            ImGui::DragFloat("HBAO Intensity", &render_setting.HBAOIntensity, 0.01f, 0.0f, 1.0f);
            ImGui::DragFloat("HBAO Radius", &render_setting.HBAORadius, 0.001f, 0.01f, 5.0f);
            ImGui::DragFloat("HBAOAngle Bias", &render_setting.HBAOAngleBias, 0.001f, 0.0f, 0.5f);
            ImGui::DragFloat("HBAO MaxRadius Pixels", &render_setting.HBAOMaxRadiusPixels, 1.0f, 16.f,128.f);

        }

        if (ImGui::CollapsingHeader("Bloom Settings"))
        {
            ImGui::DragFloat("Bloom Threshold", &render_setting.bloomThreshold, 0.01f, 0.0f, 10.0f);
            ImGui::DragInt("Bloom BlurTimes", &render_setting.bloomBlurTimes, 1, 0, 10);
            ImGui::DragInt("Bloom KernalSize", &render_setting.bloomKernalSize, 1, 0, 10);
        }

        if (ImGui::CollapsingHeader("ColorGrading Settings"))
        {
            ImGui::DragFloat("ColorGrading Effect", &render_setting.useColorGrading, 0.01f, 0.0f, 1.0f);
            ImGui::DragFloat("Brightness", &render_setting.brightness, 0.001f, -0.5f, 0.5f);
            ImGui::DragFloat("Contrast", &render_setting.contrast, 0.001f, 0.5f, 1.5f);
            ImGui::DragFloat("Saturation", &render_setting.saturation, 0.01f, -4.f, 4.f);
            ImGui::DragFloat("Temperature", &render_setting.temperature, 0.01f, -0.4f, 0.4f);
        }

        if (ImGui::CollapsingHeader("Vignette Settings"))
        {
            ImGui::DragFloat("Vignette Intensity", &render_setting.vignetteExponent, 0.01f, 0.0f, 2.0f);
            ImGui::DragFloat("Vignette Radius", &render_setting.vignetteCutoff, 0.01f, 0.0f, 1.0f);
        }
