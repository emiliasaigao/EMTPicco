...
namespace Piccolo
{
    ...
    VkSampler                               VulkanUtil::m_nearest_repeat_sampler = VK_NULL_HANDLE;
    ...
    VkSampler VulkanUtil::getOrCreateNearestRepeatSampler(VkPhysicalDevice physical_device, VkDevice device)
    {
        if (m_nearest_repeat_sampler == VK_NULL_HANDLE)
        {
            VkPhysicalDeviceProperties physical_device_properties {};
            vkGetPhysicalDeviceProperties(physical_device, &physical_device_properties);
            ...
            if (vkCreateSampler(device, &samplerInfo, nullptr, &m_nearest_repeat_sampler) != VK_SUCCESS)
            {
    ...
    VkSampler VulkanUtil::getOrCreateNearestSampler(VkPhysicalDevice physical_device, VkDevice device)
    {
        if (m_nearest_sampler == VK_NULL_HANDLE)
        {
            ...
            samplerInfo.compareOp               = VK_COMPARE_OP_ALWAYS;
            samplerInfo.minLod                  = 0.0f;
            samplerInfo.maxLod                  = 0.0f; // todo: m_irradiance_texture_miplevels
            samplerInfo.borderColor             = VK_BORDER_COLOR_INT_OPAQUE_BLACK;
            samplerInfo.unnormalizedCoordinates = VK_FALSE;
