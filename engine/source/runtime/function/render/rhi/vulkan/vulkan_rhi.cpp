...
#error Unknown Compiler
    ...
    void VulkanRHI::prepareContext()
    {
        ...
        m_current_post_process_command_buffer = m_post_process_command_buffers[m_current_frame_index];
    ...
    void VulkanRHI::createCommandBuffers()
    {
        ...
        for (uint32_t i = 0; i < s_max_frames_in_flight; ++i)
        {
            ...
            if (vkAllocateCommandBuffers(m_device, &command_buffer_allocate_info, &m_post_process_command_buffers[i]) != VK_SUCCESS)
            {
                throw std::runtime_error("vk allocate command buffers");
            }
            if (vkAllocateCommandBuffers(m_device, &command_buffer_allocate_info, &m_bloom_blur_compute_command_buffers[i]) != VK_SUCCESS)
            {
                throw std::runtime_error("vk allocate command buffers");
            }
        }
    ...
    void VulkanRHI::createDescriptorPool()
    {
        ...
        pool_sizes[0].descriptorCount = 3 + 1 + 1 + 3 + 3 + 3 + 2;
        ...
        pool_sizes[5].descriptorCount = 1 + 2;
        ...
        for (uint32_t i = 0; i < s_max_frames_in_flight; i++)
        {
            if (vkCreateSemaphore(
                ...
                vkCreateSemaphore(
                    ...
                    VK_SUCCESS ||
                vkCreateSemaphore(
                    m_device, &semaphore_create_info, nullptr, &m_image_available_for_bloom_blur_semaphores[i]) !=
                    VK_SUCCESS ||
                vkCreateSemaphore(
                    m_device, &semaphore_create_info, nullptr, &m_image_available_for_post_process_semaphores[i]) !=
                    VK_SUCCESS ||
    ...
    void VulkanRHI::createFramebufferImageAndView()
    {
        ...
        m_depth_stencil_image_view = VulkanUtil::createImageView(
            m_device, m_depth_image, m_depth_image_format, VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, 1);
        m_depth_only_image_view = VulkanUtil::createImageView(
            m_device, m_depth_image, m_depth_image_format, VK_IMAGE_ASPECT_DEPTH_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, 1);
        m_stencil_only_image_view = VulkanUtil::createImageView(
            m_device, m_depth_image, m_depth_image_format, VK_IMAGE_ASPECT_STENCIL_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, 1);
    }
    ...
    void VulkanRHI::recreateSwapchain()
    {
        ...
        vkDestroyImageView(m_device, m_depth_stencil_image_view, NULL);
        vkDestroyImageView(m_device, m_depth_only_image_view, NULL);
        vkDestroyImageView(m_device, m_stencil_only_image_view, NULL);
