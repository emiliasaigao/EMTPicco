...
#include <pcf_mask_blur_vert.h>
#include <pcf_mask_blur_frag.h>

#include <stdexcept>

namespace Piccolo
{
    void PCFMaskBlurPass::initialize(const RenderPassInitInfo* init_info)
    {
    ...
    void PCFMaskBlurPass::draw() { 
        ...
        if (m_vulkan_rhi->isDebugLabelEnabled())
        {
            VkDebugUtilsLabelEXT label_info = {
                VK_STRUCTURE_TYPE_DEBUG_UTILS_LABEL_EXT, NULL, "PCF Mask Blur", {1.0f, 1.0f, 1.0f, 1.0f}};
            m_vulkan_rhi->m_vk_cmd_begin_debug_utils_label_ext(m_vulkan_rhi->m_current_command_buffer, &label_info);
        }

        m_vulkan_rhi->m_vk_cmd_bind_pipeline(m_vulkan_rhi->m_current_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_render_pipelines[0].pipeline);
    ...
    void PCFMaskBlurPass::setupAttachments()
    {
    ...
    void PCFMaskBlurPass::setupRenderPass()
    {
        VkAttachmentDescription attachments_dscp[1] = {};

        VkAttachmentDescription& pcf_mask_blur_attachment_description = attachments_dscp[0];
        pcf_mask_blur_attachment_description.format                   = m_framebuffer.attachments[0].format;
        pcf_mask_blur_attachment_description.samples                  = VK_SAMPLE_COUNT_1_BIT;
        pcf_mask_blur_attachment_description.loadOp                   = VK_ATTACHMENT_LOAD_OP_CLEAR;
        pcf_mask_blur_attachment_description.storeOp                  = VK_ATTACHMENT_STORE_OP_STORE;
        pcf_mask_blur_attachment_description.stencilLoadOp            = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        pcf_mask_blur_attachment_description.stencilStoreOp           = VK_ATTACHMENT_STORE_OP_DONT_CARE;
        pcf_mask_blur_attachment_description.initialLayout            = VK_IMAGE_LAYOUT_UNDEFINED;
        pcf_mask_blur_attachment_description.finalLayout              = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        VkSubpassDescription subpasses[1] = {};

        VkAttachmentReference pcf_mask_blur_pass_color_attachments_reference {};
        pcf_mask_blur_pass_color_attachments_reference.attachment =
            &pcf_mask_blur_attachment_description - attachments_dscp;
        pcf_mask_blur_pass_color_attachments_reference.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

        VkSubpassDescription& pcf_mask_blur_pass = subpasses[0];
        pcf_mask_blur_pass.pipelineBindPoint     = VK_PIPELINE_BIND_POINT_GRAPHICS;
        pcf_mask_blur_pass.inputAttachmentCount  = 0;
        pcf_mask_blur_pass.pInputAttachments     = NULL;
        pcf_mask_blur_pass.colorAttachmentCount  = 1;
        pcf_mask_blur_pass.pColorAttachments     = &pcf_mask_blur_pass_color_attachments_reference;
        pcf_mask_blur_pass.pDepthStencilAttachment       = NULL;
        pcf_mask_blur_pass.preserveAttachmentCount       = 0;
        pcf_mask_blur_pass.pPreserveAttachments          = NULL;

        VkSubpassDependency dependencies[2] = {};

        VkSubpassDependency& pcf_mask_blur_pass_depend_on_pcf_mask_gen_pass = dependencies[0];
        pcf_mask_blur_pass_depend_on_pcf_mask_gen_pass.srcSubpass           = VK_SUBPASS_EXTERNAL;
        pcf_mask_blur_pass_depend_on_pcf_mask_gen_pass.dstSubpass           = 0;
        pcf_mask_blur_pass_depend_on_pcf_mask_gen_pass.srcStageMask         = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
        pcf_mask_blur_pass_depend_on_pcf_mask_gen_pass.dstStageMask         = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
        pcf_mask_blur_pass_depend_on_pcf_mask_gen_pass.srcAccessMask        = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT; // STORE_OP_STORE
        pcf_mask_blur_pass_depend_on_pcf_mask_gen_pass.dstAccessMask        = VK_ACCESS_SHADER_READ_BIT;
        pcf_mask_blur_pass_depend_on_pcf_mask_gen_pass.dependencyFlags      = 0; // NOT BY REGION
    ...
    void PCFMaskBlurPass::setupFramebuffer()
    {
    ...
    void PCFMaskBlurPass::setupDescriptorSetLayout()
    {
        m_descriptor_infos.resize(1);
        
        VkDescriptorSetLayoutBinding pcf_mask_blur_layout_bindings[1];

        VkDescriptorSetLayoutBinding& pcf_mask_blur_layout_src_texture_binding = pcf_mask_blur_layout_bindings[0];
        pcf_mask_blur_layout_src_texture_binding.binding            = 0;
        pcf_mask_blur_layout_src_texture_binding.descriptorType     = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        pcf_mask_blur_layout_src_texture_binding.descriptorCount    = 1;
        pcf_mask_blur_layout_src_texture_binding.stageFlags         = VK_SHADER_STAGE_FRAGMENT_BIT;
        pcf_mask_blur_layout_src_texture_binding.pImmutableSamplers = NULL;

        VkDescriptorSetLayoutCreateInfo pcf_mask_blur_layout_create_info {};
        pcf_mask_blur_layout_create_info.sType        = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
        pcf_mask_blur_layout_create_info.bindingCount = 1;
        pcf_mask_blur_layout_create_info.pBindings    = pcf_mask_blur_layout_bindings;

        if (VK_SUCCESS !=
            vkCreateDescriptorSetLayout(
                m_vulkan_rhi->m_device, &pcf_mask_blur_layout_create_info, NULL, &m_descriptor_infos[0].layout))
        {
    ...
    void PCFMaskBlurPass::setupPipelines()
    {
        ...
        VkShaderModule vert_shader_module =
            VulkanUtil::createShaderModule(m_vulkan_rhi->m_device, PCF_MASK_BLUR_VERT);
        VkShaderModule frag_shader_module =
            VulkanUtil::createShaderModule(m_vulkan_rhi->m_device, PCF_MASK_BLUR_FRAG);
    ...
    void PCFMaskBlurPass::setupDescriptorSet()
    {
        ...
        if (VK_SUCCESS != vkAllocateDescriptorSets(m_vulkan_rhi->m_device,
                                                    ...
                                                    &m_descriptor_infos[0].descriptor_set))
        {
            throw std::runtime_error("allocate pcf_mask_blur descriptor set");
        }

        VkDescriptorImageInfo src_pcf_mask_input_attachment_info = {};
        src_pcf_mask_input_attachment_info.sampler =
            VulkanUtil::getOrCreateNearestSampler(m_vulkan_rhi->m_physical_device, m_vulkan_rhi->m_device);
        src_pcf_mask_input_attachment_info.imageView = m_pcf_mask_gen_image_view;
        src_pcf_mask_input_attachment_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        VkWriteDescriptorSet pcf_mask_descriptor_writes_info[1];
        ...
        pcf_mask_descriptor_writes_info[0].pImageInfo      = &src_pcf_mask_input_attachment_info;

        vkUpdateDescriptorSets(m_vulkan_rhi->m_device, 1, pcf_mask_descriptor_writes_info, 0, NULL);
