...
namespace Piccolo
{
    void RenderResource::uploadGlobalRenderResource(std::shared_ptr<RHI> rhi, LevelResourceDesc level_resource_desc)
    {
        ...
        // SSAO Generate Noise 
        std::shared_ptr<TextureData> ssao_generate_noise = std::make_shared<TextureData>();
        std::uniform_real_distribution<float> randomFloats(0.0, 1.0);
        std::default_random_engine            generator;
        Vector4* data = new Vector4[64];
        for (int i = 0; i < 64; i++)
        {
            data[i] = Vector4(randomFloats(generator) * 2.0 - 1.0, randomFloats(generator) * 2.0 - 1.0, 0.0f, 0.0f);
        }
        ssao_generate_noise->m_pixels            = (void*)data;
        ssao_generate_noise->m_width             = 8;
        ssao_generate_noise->m_height            = 8;
        ssao_generate_noise->m_format            = PICCOLO_PIXEL_FORMAT::PICCOLO_PIXEL_FORMAT_R32G32B32A32_FLOAT;
        ssao_generate_noise->m_depth             = 1;
        ssao_generate_noise->m_array_layers      = 1;
        ssao_generate_noise->m_mip_levels        = 1;
        ssao_generate_noise->m_type              = PICCOLO_IMAGE_TYPE::PICCOLO_IMAGE_TYPE_2D;

        VulkanUtil::createGlobalImage(
            rhi.get(),
            m_global_render_resource._ssao_noise_resource._ssao_noise_texture_image,
            m_global_render_resource._ssao_noise_resource._ssao_noise_texture_image_view,
            m_global_render_resource._ssao_noise_resource._ssao_noise_texture_image_allocation,
            ssao_generate_noise->m_width,
            ssao_generate_noise->m_height,
            ssao_generate_noise->m_pixels,
            ssao_generate_noise->m_format,
            1);
    }
                                              ...
                                              std::shared_ptr<RenderCamera> camera)
    {
        ...
        uint32_t window_width = raw_rhi->m_swapchain_extent.width;
        uint32_t window_height = raw_rhi->m_swapchain_extent.height;

        Matrix4x4 view_matrix = camera->getViewMatrix();
        Matrix4x4 proj_matrix = camera->getPersProjMatrix();
        Vector3   camera_position = camera->position();
        Matrix4x4 proj_inv_matrix  = proj_matrix.inverse();
        ...
        m_particle_collision_perframe_storage_buffer_object.proj_inv_matrix  = proj_inv_matrix;
        ...
        m_nbr_mesh_perframe_storage_buffer_object.camera_position  = camera_position;
        m_nbr_mesh_perframe_storage_buffer_object.proj_matrix      = proj_matrix;
        m_nbr_mesh_perframe_storage_buffer_object.view_matrix      = view_matrix;
        m_nbr_mesh_perframe_storage_buffer_object.proj_view_matrix = proj_view_matrix;
        m_nbr_mesh_perframe_storage_buffer_object.scene_directional_light.direction =
            render_scene->m_directional_light.m_direction.normalisedCopy();
        m_nbr_mesh_perframe_storage_buffer_object.scene_directional_light.color = render_scene->m_directional_light.m_color;

        m_nbr_outline_mesh_perframe_storage_buffer_object.proj_view_matrix = proj_view_matrix;
        m_nbr_outline_mesh_perframe_storage_buffer_object.proj_matrix = proj_matrix;
        m_nbr_outline_mesh_perframe_storage_buffer_object.view_matrix = view_matrix;
        m_nbr_outline_mesh_perframe_storage_buffer_object.cameraFOV = camera->getFOV().getY();

        // SSAO Generate
        //m_ssao_generate_perframe_storage_buffer_object.projection = proj_matrix;
        //m_ssao_generate_perframe_storage_buffer_object.view = view_matrix;
        //std::uniform_real_distribution<float> randomFloats(0.0, 1.0);
        //std::default_random_engine generator;
        //for (int i = 0; i < 64; ++i) {
        //    Vector3 ssao_sample = Vector3(randomFloats(generator) * 2.0f - 1.0f, randomFloats(generator) * 2.0f - 1.0f, randomFloats(generator));
        //    ssao_sample.normalise();
        //    ssao_sample *= randomFloats(generator);
        //    float scale = float(i) / 64.0;

        //    scale = 0.1 + scale * scale * (1.0f - 0.1f);
        //    ssao_sample *= scale;
        //    m_ssao_generate_perframe_storage_buffer_object.samples[i] = Vector4(ssao_sample, 0.0);
        //}

        //m_ssao_generate_perframe_storage_buffer_object.windowWidth     = window_width;
        //m_ssao_generate_perframe_storage_buffer_object.windowHeight    = window_height;
        //m_ssao_generate_perframe_storage_buffer_object.SSAOEffect = m_render_global_effect_setting_object.SSAOEffect;
        //m_ssao_generate_perframe_storage_buffer_object.SSAORadius = m_render_global_effect_setting_object.SSAORadius;
        //m_ssao_generate_perframe_storage_buffer_object.SSAOBias   = m_render_global_effect_setting_object.SSAOBias;
        //m_ssao_generate_perframe_storage_buffer_object.SSAOKernelSize = m_render_global_effect_setting_object.SSAOKernelSize;
        m_ssao_generate_perframe_storage_buffer_object.proj_matrix = proj_matrix;
        m_ssao_generate_perframe_storage_buffer_object.proj_inv_matrix = proj_inv_matrix;

        m_ssao_generate_perframe_storage_buffer_object.view_matrix = view_matrix;
        m_ssao_generate_perframe_storage_buffer_object.windowWidth     = window_width;
        m_ssao_generate_perframe_storage_buffer_object.windowHeight    = window_height;
        m_ssao_generate_perframe_storage_buffer_object.intensity = m_render_global_effect_setting_object.HBAOIntensity;
        m_ssao_generate_perframe_storage_buffer_object.radius = 1.5f * m_render_global_effect_setting_object.HBAORadius;
        m_ssao_generate_perframe_storage_buffer_object.angle_bias   = m_render_global_effect_setting_object.HBAOAngleBias;
        m_ssao_generate_perframe_storage_buffer_object.max_radius_pixels = m_render_global_effect_setting_object.HBAOMaxRadiusPixels;
        m_ssao_generate_perframe_storage_buffer_object.neg_inv_radius2 = -1.0 / (m_render_global_effect_setting_object.HBAORadius * m_render_global_effect_setting_object.HBAORadius);
        float tanHalfFOVy = Math::tan(Math::degreesToRadians(camera->getFOV().getY() * 0.5f));
        m_ssao_generate_perframe_storage_buffer_object.radius_pixel =
            window_height * m_render_global_effect_setting_object.HBAORadius * 1.5f / tanHalfFOVy / 2.0f;
                                                                 ...
                                                                 RenderMaterialData   material_data)
    {
        ...
        if (entity.m_is_NBR_material)
        {
            auto it = m_vulkan_nbr_materials.find(assetid);
            if (it != m_vulkan_nbr_materials.end())
            {
                return it->second;
            }
            else
            {
                VulkanNBRMaterial temp;
                auto              res = m_vulkan_nbr_materials.insert(std::make_pair(assetid, std::move(temp)));
                assert(res.second);
                ...
                void*                light_map_image_pixels = empty_image;
                uint32_t             light_map_width        = 1;
                uint32_t             light_map_height       = 1;
                PICCOLO_PIXEL_FORMAT light_map_format =
                    PICCOLO_PIXEL_FORMAT::PICCOLO_PIXEL_FORMAT_R8G8B8A8_UNORM;
                if (material_data.m_light_map_texture)
                {
                    light_map_image_pixels = material_data.m_light_map_texture->m_pixels;
                    light_map_width =
                        static_cast<uint32_t>(material_data.m_light_map_texture->m_width);
                    light_map_height =
                        static_cast<uint32_t>(material_data.m_light_map_texture->m_height);
                    light_map_format = material_data.m_light_map_texture->m_format;
                }

                void*                ramp_warm_image_pixels = empty_image;
                uint32_t             ramp_warm_width        = 1;
                uint32_t             ramp_warm_height       = 1;
                PICCOLO_PIXEL_FORMAT ramp_warm_format       = PICCOLO_PIXEL_FORMAT::PICCOLO_PIXEL_FORMAT_R8G8B8A8_SRGB;
                if (material_data.m_ramp_warm_texture)
                {
                    ramp_warm_image_pixels = material_data.m_ramp_warm_texture->m_pixels;
                    ramp_warm_width        = static_cast<uint32_t>(material_data.m_ramp_warm_texture->m_width);
                    ramp_warm_height       = static_cast<uint32_t>(material_data.m_ramp_warm_texture->m_height);
                    ramp_warm_format       = material_data.m_ramp_warm_texture->m_format;
                }

                void*                ramp_cool_image_pixels = empty_image;
                uint32_t             ramp_cool_image_width  = 1;
                uint32_t             ramp_cool_image_height = 1;
                PICCOLO_PIXEL_FORMAT ramp_cool_image_format = PICCOLO_PIXEL_FORMAT::PICCOLO_PIXEL_FORMAT_R8G8B8A8_SRGB;
                if (material_data.m_ramp_cool_texture)
                {
                    ramp_cool_image_pixels = material_data.m_ramp_cool_texture->m_pixels;
                    ramp_cool_image_width  = static_cast<uint32_t>(material_data.m_ramp_cool_texture->m_width);
                    ramp_cool_image_height = static_cast<uint32_t>(material_data.m_ramp_cool_texture->m_height);
                    ramp_cool_image_format = material_data.m_ramp_cool_texture->m_format;
                }

                void*                face_map_image_pixels = empty_image;
                uint32_t             face_map_image_width  = 1;
                uint32_t             face_map_image_height = 1;
                PICCOLO_PIXEL_FORMAT face_map_image_format = PICCOLO_PIXEL_FORMAT::PICCOLO_PIXEL_FORMAT_R8G8B8A8_UNORM;
                if (material_data.m_face_map_texture)
                {
                    face_map_image_pixels = material_data.m_face_map_texture->m_pixels;
                    face_map_image_width  = static_cast<uint32_t>(material_data.m_face_map_texture->m_width);
                    face_map_image_height = static_cast<uint32_t>(material_data.m_face_map_texture->m_height);
                    face_map_image_format = material_data.m_face_map_texture->m_format;
                }

                VulkanNBRMaterial& now_material = res.first->second;
                ...
                // data
                {
                    ...
                    VkDeviceSize buffer_size = sizeof(MeshPerNBRMaterialUniformBufferObject);
                    ...
                    MeshPerNBRMaterialUniformBufferObject& nbr_material_uniform_buffer_info =
                        (*static_cast<MeshPerNBRMaterialUniformBufferObject*>(staging_buffer_data));

                    nbr_material_uniform_buffer_info._area                              = entity.m_area;
                    nbr_material_uniform_buffer_info._front_face_tint_color             = entity.m_front_face_tint_color;
                    nbr_material_uniform_buffer_info._alpha                             = entity.m_alpha;
                    nbr_material_uniform_buffer_info._back_face_tint_color              = entity.m_back_face_tint_color;
                    nbr_material_uniform_buffer_info._alpha_clip                        = entity.m_alpha_clip;
                    nbr_material_uniform_buffer_info._indirect_light_usage              = entity.m_indirect_light_usage;
                    nbr_material_uniform_buffer_info._indirect_light_mix_base_color     = entity.m_indirect_light_mix_base_color;
                    nbr_material_uniform_buffer_info._indirect_light_occlusion_usage    = entity.m_indirect_light_occlusion_usage;
                    nbr_material_uniform_buffer_info._main_light_color_usage            = entity.m_main_light_color_usage;
                    nbr_material_uniform_buffer_info._shadow_threshold_center           = entity.m_shadow_threshold_center;
                    nbr_material_uniform_buffer_info._shadow_threshold_softness         = entity.m_shadow_threshold_softness;
                    nbr_material_uniform_buffer_info._shadow_ramp_offset                = entity.m_shadow_ramp_offset;
                    nbr_material_uniform_buffer_info._face_shadow_offset                = entity.m_face_shadow_offset;
                    nbr_material_uniform_buffer_info._face_shadow_transition_softness   = entity.m_face_shadow_transition_softness;
                    nbr_material_uniform_buffer_info._specular_exponent                 = entity.m_specular_exponent;
                    nbr_material_uniform_buffer_info._specular_Ks_non_metal             = entity.m_specular_Ks_non_metal;
                    nbr_material_uniform_buffer_info._specular_Ks_metal                 = entity.m_specular_Ks_metal;
                    nbr_material_uniform_buffer_info._specular_brightness               = entity.m_specular_brightness;
                    nbr_material_uniform_buffer_info._rim_light_width                   = entity.m_rim_light_width;
                    nbr_material_uniform_buffer_info._rim_light_threshold               = entity.m_rim_light_threshold;
                    nbr_material_uniform_buffer_info._rim_light_fadeout                 = entity.m_rim_light_fadeout;
                    nbr_material_uniform_buffer_info._rim_light_brightness              = entity.m_rim_light_brightness;
                    nbr_material_uniform_buffer_info._rim_light_tint_color              = entity.m_rim_light_tint_color;
                    nbr_material_uniform_buffer_info._rim_light_mix_albedo              = entity.m_rim_light_mix_albedo;
                    nbr_material_uniform_buffer_info._emission_tint_color               = entity.m_emission_tint_color;
                    nbr_material_uniform_buffer_info._emission_mix_base_color           = entity.m_emission_mix_base_color;
                    nbr_material_uniform_buffer_info._emission_intensity                = entity.m_emission_intensity;
                ...
                TextureDataToUpdateNBR update_texture_data;
                ...
                update_texture_data.light_map_image_pixels          = light_map_image_pixels;
                update_texture_data.light_map_image_width           = light_map_width;
                update_texture_data.light_map_image_height          = light_map_height;
                update_texture_data.light_map_image_format          = light_map_format;
                update_texture_data.ramp_warm_image_pixels          = ramp_warm_image_pixels;
                update_texture_data.ramp_warm_image_width           = ramp_warm_width;
                update_texture_data.ramp_warm_image_height          = ramp_warm_height;
                update_texture_data.ramp_warm_image_format          = ramp_warm_format;
                update_texture_data.ramp_cool_image_pixels          = ramp_cool_image_pixels;
                update_texture_data.ramp_cool_image_width           = ramp_cool_image_width;
                update_texture_data.ramp_cool_image_height          = ramp_cool_image_height;
                update_texture_data.ramp_cool_image_format          = ramp_cool_image_format;
                update_texture_data.face_map_image_pixels           = face_map_image_pixels;
                update_texture_data.face_map_image_width            = face_map_image_width;
                update_texture_data.face_map_image_height           = face_map_image_height;
                update_texture_data.face_map_image_format           = face_map_image_format;
                update_texture_data.now_material                    = &now_material;

                updateTextureImageDataNBR(rhi, update_texture_data);
                ...
                material_uniform_buffer_info.range                  = sizeof(MeshPerNBRMaterialUniformBufferObject);
                ...
                VkDescriptorImageInfo light_map_image_info = {};
                light_map_image_info.imageLayout           = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                light_map_image_info.imageView             = now_material.light_map_texture_image_view;
                light_map_image_info.sampler =
                    VulkanUtil::getOrCreateMipmapSampler(vulkan_context->m_physical_device,
                                                         vulkan_context->m_device,
                                                         light_map_width,
                                                         light_map_height);

                VkDescriptorImageInfo ramp_warm_image_info = {};
                ramp_warm_image_info.imageLayout           = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                ramp_warm_image_info.imageView             = now_material.ramp_warm_texture_view;
                ramp_warm_image_info.sampler =
                    VulkanUtil::getOrCreateNearestSampler(vulkan_context->m_physical_device,
                                                         vulkan_context->m_device);

                VkDescriptorImageInfo ramp_cool_image_info = {};
                ramp_cool_image_info.imageLayout           = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                ramp_cool_image_info.imageView             = now_material.ramp_cool_texture_view;
                ramp_cool_image_info.sampler = VulkanUtil::getOrCreateNearestSampler(vulkan_context->m_physical_device,
                                                                                    vulkan_context->m_device);

                VkDescriptorImageInfo face_map_image_info = {};
                face_map_image_info.imageLayout           = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                face_map_image_info.imageView             = now_material.face_map_view;
                face_map_image_info.sampler = VulkanUtil::getOrCreateMipmapSampler(vulkan_context->m_physical_device,
                                                                                   vulkan_context->m_device,
                                                                                   face_map_image_width,
                                                                                   face_map_image_height);
            ...
            else
            {
                ...
                if (material_data.m_metallic_roughness_texture)
                {
                    ...
                    metallic_roughness_width =
                        static_cast<uint32_t>(material_data.m_metallic_roughness_texture->m_width);
                    metallic_roughness_height =
                        static_cast<uint32_t>(material_data.m_metallic_roughness_texture->m_height);
                ...
                emissive_image_info.sampler = VulkanUtil::getOrCreateMipmapSampler(vulkan_context->m_physical_device,
                                                                                   vulkan_context->m_device,
                                            ...
                                            VulkanMesh&                            now_mesh)
    {
        ...
        if (enable_vertex_blending)
        {
            ...
            VkDeviceSize vertex_color_buffer_size = sizeof(MeshVertex::VulkanMeshVertexColor) * vertex_count;
            ...
            VkDeviceSize vertex_color_buffer_offset = vertex_varying_buffer_offset + vertex_varying_buffer_size;
            VkDeviceSize vertex_joint_binding_buffer_offset = vertex_color_buffer_offset + vertex_color_buffer_size;

            // temporary staging buffer
            VkDeviceSize inefficient_staging_buffer_size =
                vertex_position_buffer_size + vertex_varying_enable_blending_buffer_size + vertex_varying_buffer_size + vertex_color_buffer_size +
            ...
            MeshVertex::VulkanMeshVertexColor* mesh_vertex_colors =
                reinterpret_cast<MeshVertex::VulkanMeshVertexColor*>(
                    reinterpret_cast<uintptr_t>(inefficient_staging_buffer_data) + vertex_color_buffer_offset);
            ...
            for (uint32_t vertex_index = 0; vertex_index < vertex_count; ++vertex_index)
            {
                ...
                mesh_vertex_colors[vertex_index].color              = Vector3(vertex_buffer_data[vertex_index].cx,
                                                                 vertex_buffer_data[vertex_index].cy,
                                                                 vertex_buffer_data[vertex_index].cz);
            ...
            vmaCreateBuffer(vulkan_context->m_assets_allocator,
                            ...
                            NULL);
            bufferInfo.size = vertex_color_buffer_size;
            vmaCreateBuffer(vulkan_context->m_assets_allocator,
                            &bufferInfo,
                            &allocInfo,
                            &now_mesh.mesh_vertex_color_buffer,
                            &now_mesh.mesh_vertex_color_buffer_allocation,
                            NULL);
    ...
    void RenderResource::updateTextureImageDataNBR(std::shared_ptr<RHI> rhi, const TextureDataToUpdateNBR& texture_data)
    {
        VulkanUtil::createGlobalImage(rhi.get(),
                                      texture_data.now_material->base_color_texture_image,
                                      texture_data.now_material->base_color_image_view,
        ...
        VulkanUtil::createGlobalImage(rhi.get(),
                                      texture_data.now_material->light_map_texture_image,
                                      texture_data.now_material->light_map_texture_image_view,
                                      texture_data.now_material->light_map_texture_image_allocation,
                                      texture_data.light_map_image_width,
                                      texture_data.light_map_image_height,
                                      texture_data.light_map_image_pixels,
                                      texture_data.light_map_image_format);

        VulkanUtil::createGlobalImage(rhi.get(),
                                      texture_data.now_material->ramp_warm_texture_image,
                                      texture_data.now_material->ramp_warm_texture_view,
                                      texture_data.now_material->ramp_warm_texture_allocation,
                                      texture_data.ramp_warm_image_width,
                                      texture_data.ramp_warm_image_height,
                                      texture_data.ramp_warm_image_pixels,
                                      texture_data.ramp_warm_image_format);

        VulkanUtil::createGlobalImage(rhi.get(),
                                      texture_data.now_material->ramp_cool_texture_image,
                                      texture_data.now_material->ramp_cool_texture_view,
                                      texture_data.now_material->ramp_cool_texture_allocation,
                                      texture_data.ramp_cool_image_width,
                                      texture_data.ramp_cool_image_height,
                                      texture_data.ramp_cool_image_pixels,
                                      texture_data.ramp_cool_image_format);

        VulkanUtil::createGlobalImage(rhi.get(),
                                      texture_data.now_material->face_map_texture_image,
                                      texture_data.now_material->face_map_view,
                                      texture_data.now_material->face_map_allocation,
                                      texture_data.face_map_image_width,
                                      texture_data.face_map_image_height,
                                      texture_data.face_map_image_pixels,
                                      texture_data.face_map_image_format);
    }
